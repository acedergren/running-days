<script lang="ts">
	...
	// Pace trend data - API returns min/km directly
	const paceTrendData = $derived(
		data.yearlyData
			.filter((d) => d.paceMinPerKm && d.paceMinPerKm > 0)
			.slice(-30)
			.map(
				(d): PaceDataPoint => ({
					date: new Date(d.date),
					pace: d.paceMinPerKm as number
				})
			)
	);

	// Calculate averages - API returns km and hours
	const avgDistancePerRun = $derived(
		data.stats.totalDays > 0 ? data.stats.totalDistanceKm / data.stats.totalDays : 0
	);
	const avgDurationPerRun = $derived(
		data.stats.totalDays > 0 ? (data.stats.totalDurationHours * 60) / data.stats.totalDays : 0
	);
</script>

<svelte:head>
	<title>Insights - Running Days</title>
</svelte:head>
			...
			<div class="grid grid-cols-2 md:grid-cols-4 gap-3">
				<div class="stat-card">
					...
					<div class="flex items-baseline gap-1">
						<span class="stat-value text-[var(--accent-primary)]"
							>{data.streaks.currentStreak}</span
						>
				...
				<div class="stat-card">
					...
					<div class="flex items-baseline gap-1">
						<span class="stat-value">{data.streaks.longestStreak}</span>
						<span class="stat-unit">days</span>
					</div>
				</div>
				...
				<div class="stat-card">
					...
					<div class="flex items-baseline gap-1">
						<span class="stat-value">{Math.round(avgDurationPerRun)}</span>
					...
					<div class="h-64">
						...
						>
							<Svg>
								...
								/>
								<Bars radius={6} fill="url(#bar-gradient)" strokeWidth={0} />
								<defs>
					...
					<div class="h-64">
						...
						>
							<Svg>
								...
								/>
								<Bars radius={6} fill="url(#days-gradient)" strokeWidth={0} />
								<defs>
					...
					<div class="h-64">
						...
						>
							<Svg>
								...
								/>
								<Bars radius={4} fill="url(#weekly-gradient)" strokeWidth={0} />
								<defs>
							...
							<Tooltip.Root header={(d: WeeklyDataPoint) => d.week} let:data>
								<Tooltip.Item
									...
									valueAlign="right"
								/>
								<Tooltip.Item label="Days" value={`${data.days} runs`} valueAlign="right" />
							</Tooltip.Root>
						</Chart>
					</div>
				{:else}
	...
	<section class="px-4 py-6">
		<div class="mx-auto max-w-2xl">
			...
			<div class="grid grid-cols-2 gap-3">
				<div
					class="rounded-2xl border border-[oklch(0.8_0.18_85_/_0.3)] bg-gradient-to-br from-[oklch(0.8_0.18_85_/_0.1)] to-transparent p-5"
				>
					...
					<div class="flex items-baseline gap-1">
						<span class="font-display text-3xl font-bold text-[oklch(0.85_0.16_85)]">
							{data.stats.bestDistanceKm.toFixed(1)}
						</span>
				...
				<div
					class="rounded-2xl border border-[var(--accent-primary)]/30 bg-gradient-to-br from-[var(--accent-primary)]/10 to-transparent p-5"
				>
					...
					<div class="flex items-baseline gap-1">
						<span class="font-display text-3xl font-bold text-[var(--accent-primary)]">
							{#if data.stats.bestPaceMinPerKm}
								{Math.floor(data.stats.bestPaceMinPerKm)}:{String(
									Math.round((data.stats.bestPaceMinPerKm % 1) * 60)
								).padStart(2, '0')}
							{:else}
								--
							{/if}
